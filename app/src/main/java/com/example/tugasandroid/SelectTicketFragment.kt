...
class SelectTicketFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(binding){
            val args = SelectTicketFragmentArgs.fromBundle(arguments as Bundle)
            txtTicket.text = args.ticketType

            edtTicket.setOnClickListener{
                val action = SelectTicketFragmentDirections.actionSelectTicketFragmentToSpinnerTicketFragment()
                view.findNavController().navigate(action)
            }

            findNavController().currentBackStackEntry?.savedStateHandle?.let {
                handle -> handle.getLiveData<String>("ticketType")
